...
def parse():
    ...
    parser.add_argument('--trainMask_path', default='data/trainMask.txt')
    parser.add_argument('--trainNoMask_path', default='data/trainNoMask.txt')
    parser.add_argument('--testMask_path', default='data/testMask.txt')
    parser.add_argument('--testNoMask_path', default='data/testNoMask.txt')
    # model setting
...
class Model_factory(pl.LightningModule):
    ...
    def test_step(self, batch):
        img, sf_img = batch
        fake_img, z = self.generate(sf_img)
...
def main():
    ...
    trainDataset = HippoDataset(folder=args.root, MaskList_path=args.trainMask_path, noMaskList_path=args.trainNoMask_path, train=True)
    trainDataloader = DataLoader(dataset=trainDataset,
                                 batch_size=args.batch_size,
                                 shuffle=True, 
                                 num_workers=cpu_count())
    valDataset = HippoDataset(folder=args.root, MaskList_path=args.testMask_path, noMaskList_path=args.testNoMask_path, train=False)
...
#     trainer = pl.Trainer(fast_dev_run=True, logger=tb_logger, accelerator='gpu', devices=[1, 2])
    trainer = pl.Trainer(max_epochs=args.epochs, check_val_every_n_epoch=5,
                         ...
                         accelerator='gpu', devices=[1, 2])
    if args.train:
        trainer.fit(model=Model, train_dataloaders=trainDataloader, val_dataloaders=valDataloader)
    elif args.resume:
        trainer.fit(model=Model, train_dataloaders=trainDataloader, val_dataloaders=valDataloader, ckpt_path=args.resume_path)
